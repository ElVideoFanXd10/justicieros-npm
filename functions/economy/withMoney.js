...
async function withMoneyGlobal(userid, money){
    const schemaDB = await moneyDB.findOne({ user: userid })

    if(!schemaDB)
    throw new justicierosError(`Un error inesperado a ocurrido: El usuario no puede retirar dinero porque no a usado el cliente antes de ocupar este comando, Codigo de error: ${errorEco}`)
    if(!userid){
